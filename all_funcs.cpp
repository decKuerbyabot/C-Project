...
void block::player_attack_CM(map<vector<int>, block*>& gameboard, char oper ){
	...
	case 'e':
		if (player_potion) {
			cout << "Your life was prolonged." << endl;
			life += 3;
			player_potion--;
		}
		else {
	...
	case 'q':
		...
		case 'M':
			if (saberlife) {
				...
				cout << "Monster " << name << " is damaged by hand." << endl;
			}
		...
		case 'C':
			...
			if (gameboard[pos]->getlife() <= 0) {
				cout << "You opened chest " << name << endl;
				if (gameboard[pos]->getkey()) {
					cout << "You got the key." << endl;
					cout << "Keep on conquering the monsters." << endl;
					key = true;
				}
				else if (gameboard[pos]->getpotion()) {
					cout << "You got a bottle of potion." << endl;
					player_potion++;
				}
				else if (gameboard[pos]->getsaber()) {
					if (saberlife == 0) {
						saberlife = 3;
						cout << "You got a new saber." << endl;
					}
					else {
						cout << "There is a saber in the chest" << endl;
						cout << "Your current one is still in use" << endl;
						cout << "The found saber is lost." << endl;
					}
				}
				else {
...
void move_player(map<vector<int>, block*>& gameboard, char& oper, vector<int>& playerpos) {
	...
	gameboard[playerpos]->renew_pos(playerpos);
		...
		if (type == 'W') {
			...
			cout << "You are running into monster, your lost one life point for your boldness." << endl;;
			gameboard[previouspos]->deduce_life();
		}
		else if (type == 'C') {
			cout << "You are running into chest, you can open it or ignore." << endl;;
		}
